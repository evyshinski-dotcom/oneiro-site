	...
	#oneiro-profile-popup::-webkit-scrollbar{
	  width:6px;
	}

	#oneiro-profile-popup::-webkit-scrollbar-thumb{
	  background:#d7dbe7;
	  border-radius:999px;
	}
	...
	#oneiro-profile-popup,
	#oneiro-profile-popup *,
	#oneiro-profile-popup *::before,
	#oneiro-profile-popup *::after{
	  font-family:'TildaSans', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial, sans-serif;
	  box-sizing:border-box;
	}
	...
	#oneiro-profile-popup .op-dot.is-inactive{
	  background:#b8bcc7;
	}

	#oneiro-profile-popup .op-info{
	  margin-left:18px;
	}

	#oneiro-profile-popup .op-info.op-info-plain{
	  margin-left:0;
	}
	...
	#oneiro-profile-popup .op-line:last-child{
	  border-bottom:none;
	  margin-bottom:0;
	  padding-bottom:0;
	}
	...
	#oneiro-profile-popup .op-action-btn:hover{
	  background:#e9edf8;
	}

	#oneiro-profile-popup .op-action-btn:disabled{
	  opacity:.55;
	  cursor:default;
	  transform:none;
	}

	#oneiro-profile-popup .op-btn-icon{
	  display:inline-flex;
	  align-items:center;
	  justify-content:center;
	  width:16px;
	...
	#oneiro-profile-popup .op-btn-icon svg{
	  display:block;
	  width:16px;
	  height:16px;
	  stroke:currentColor;
	...
	#oneiro-profile-popup .op-buy-star{
	  font-size:14px;
	  line-height:1;
	}
...
#oneiro-profile-popup .op-certificate-actions .op-action-btn{
  width:100%;
	...
	#oneiro-profile-popup .op-child:last-child{
	  margin-bottom:0;
	}

	#oneiro-profile-popup .op-loading,
	#oneiro-profile-popup .op-error,
	#oneiro-profile-popup .op-empty,
	#oneiro-profile-popup .op-saving{
	  font-size:13px;
	  line-height:1.5;
	  color:#666c80;
	}
	...
	#oneiro-profile-popup .op-hidden{
	  display:none!important;
	}

	#oneiro-profile-popup .op-children-list{
	  position:relative;
	  min-height:24px;
	  overflow-wrap:anywhere;
	}

	#oneiro-profile-popup .op-children-list.is-editing{
	  background:#f7f8fc;
	  border:1px solid #d9deeb;
	  border-radius:14px;
	  padding:12px 14px;
	...
	#oneiro-profile-popup .op-children-list.is-editing:focus{
	  border-color:#bfc8e3;
	  box-shadow:0 0 0 3px rgba(191,200,227,.18);
	}

	#oneiro-profile-popup .op-children-list.is-saving{
	  opacity:.72;
	}

	#oneiro-profile-popup .op-children-list.is-editing .op-child{
	  margin-bottom:10px;
	}

	#oneiro-profile-popup .op-inline-note{
	  margin-top:10px;
	  font-size:12px;
	  line-height:1.45;
	  color:#7a8197;
	...
	#oneiro-profile-popup .op-inline-note.is-error{
	  color:#b24c63;
	}

	#oneiro-profile-popup .op-inline-hint{
	  margin-top:10px;
	  padding:10px 12px;
	  background:#f7f3ff;
	...
	#oneiro-profile-popup .op-inline-hint b{
	  font-weight:600;
	}
	  ...
	  <div class="op-wrap">
		...
		<div class="op-card">
		  ...
		  <div class="op-info op-info-plain">
			...
			<div class="op-line" id="op-tg-username-line">Имя пользователя Telegram: —</div>
		  </div>
			...
			<button class="op-action-btn" id="op-logout-btn" type="button">
			  <span class="op-btn-icon" aria-hidden="true">
				<svg viewBox="0 0 24 24" fill="none" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round">
				  <path d="M14 7V5.5C14 4.12 12.88 3 11.5 3h-5A2.5 2.5 0 0 0 4 5.5v13A2.5 2.5 0 0 0 6.5 21h5A2.5 2.5 0 0 0 14 18.5V17"/>
				  <path d="M10 12h10"/>
		...
		<div class="op-card">
		  ...
		  <div class="op-inline-hint op-hidden" id="op-children-hint">
			Все, что считаете важным. Имя, дата рождения, пол — например:
		  ...
		  <div class="op-bottom">
			<button class="op-action-btn" id="op-edit-children-btn" type="button">
			  <span class="op-btn-icon" aria-hidden="true">
				<svg viewBox="0 0 24 24" fill="none" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round">
				  <path d="M4 20h4.2l10-10a2.2 2.2 0 0 0 0-3.1l-1.1-1.1a2.2 2.2 0 0 0-3.1 0L4 15.8V20Z"/>
		...
		<div class="op-card">
		  ...
		  <div class="op-sub-body">
			...
			<div class="op-bottom op-sub-actions">
			  <a class="op-action-btn" id="op-buy-btn" href="/dev-checkout">
	...
	<script>
	(function () {
	  const PROFILE_WEBHOOK_URL = 'https://ivagulin.dedyn.io/webhook/dev/oneiro/getUserInfo';
	  const UPDATE_CHILDREN_WEBHOOK_URL = 'https://ivagulin.dedyn.io/webhook/dev/oneiro/updateBabyInfo';
	  const CANCEL_AUTORENEW_WEBHOOK_URL = 'https://ivagulin.dedyn.io/webhook/dev/oneiro/cancelAutorenew';
	  const ACTIVATE_CERTIFICATE_WEBHOOK_URL = 'https://ivagulin.dedyn.io/webhook/dev/oneiro/activateCertificate';
	  const SAVE_TIMEOUT_MS = 30000;
	  const LOGIN_PAGE_PATH = '/dev-login';
...
function bindChildrenEditing() {
	  ...
	  function bindProfilePopupOpen() {
		document.addEventListener('click', function (e) {
		  const link = e.target.closest('a[href="#popup:profile"]');
		  if (!link) return;
	  ...
	  function bindHashFallback() {
		if (window.location.hash === '#popup:profile') {
		  setTimeout(() => {
			setPopupViewportHeight();
			loadProfile(true);
		  }, 300);
